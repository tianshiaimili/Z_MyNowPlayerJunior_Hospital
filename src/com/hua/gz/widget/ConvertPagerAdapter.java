...
public abstract class ConvertPagerAdapter extends PagerAdapter {
	...
	private static final String TAG = ConvertPagerAdapter.class.getSimpleName();
	...
	// right -> left : instantiateItem -> destroyItem, if page width is 1, then ViewPager max child count is 4 and soon destroyItem to 3.
	public ConvertPagerAdapter(Context context) {
		mContext = context;
		mPages = new SparseArray<ViewGroup>();
	}
	
	// Called when the item of position not yet destroyItem() and position is at (current selected - 1) or (current selected + 1)
